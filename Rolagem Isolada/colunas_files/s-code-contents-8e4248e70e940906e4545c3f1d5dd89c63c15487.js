s_estadao = new AppMeasurement();


/************************** CONFIG SECTION **************************/
/* You may add or alter any code config here. */

s_estadao.usePlugins=true;
...
s_estadao.server= getBaseDomain();


s_estadao.doPlugins=function(s) {

	if(!s_estadao.channel) {
		var ChannelSplit=window.location.host.split(".");
		s_estadao.channel=ChannelSplit[0]+'.'+ChannelSplit[1];
	}

	if(!s_estadao.pageName)s_estadao.pageName=s_estadao.channel+window.location.pathname;
	if(s_estadao.pageName.charAt(s_estadao.pageName.length - 1) =="/" || s_estadao.pageName=="") s_estadao.pageName=s_estadao.pageName + "home";
	s_estadao.pageName=s_estadao.pageName.replace(/\//g,"|");
	
	if(s_estadao.pageName){	
		var s_hps=s_estadao.split(s_estadao.pageName,"|");
		s_hps.pop();
		s_estadao.hier1 = s_hps.join("|");
	}
	
	// Previous Page & % Page Viewed
	var ppn = s_estadao.getPreviousValue(s_estadao.pageName,'s_ppn'); //prop21: prev page name
	var ppv = s_estadao.getPercentPageViewed(s_estadao.pageName); //get array of data on prev page % viewed
	if( ppv && typeof ppv=='object' && ppv[0] == ppn ) { //if ppv array returned and prev page id matches prev page name
	  var ppv1 = ppv[1] + '|' + ppv[2]; //prop11: prev page max and initial % viewed, delimited by "|". (Use classifications to separate values)
	  var ppv2 = ppv[4] + 'x' + ppv[5]; //prop12: viewport width x height in CSS pixels
	  var ppv3 = ppv[6] + 'x' + ppv[7]; //prop13: display width x height in real pixels
	  var ppv4 = ppv[8]; //prop14: pixel density, i.e. number of virtual pixels per real pixel
	  var ppv5 = ppv[9]; //prop15: device orientation: initial and final (final only present if rotation occurred)
	}
	
	 var visnum=s_estadao.getVisitNum();
	

		var getTimeParting=s_estadao.getTimeParting('n','-6'); /* Time Parting Plug-In: Returns day of week + hour*/
		var getDateParting=s_estadao.getTimeParting('j','-6');
		var newRepeat=s_estadao.getNewRepeat(); // New vs. Repeat Visitors
		

  // Variable Mapping
  
  s_estadao.prop1 = s_hps[0];
  s_estadao.prop2 = s_hps[0] + "|" + s_hps[1];
  s_estadao.prop3 = s_hps[0] + "|" + s_hps[1] + "|" + s_hps[2];
  s_estadao.prop4 = s_hps[0] + "|" + s_hps[1] + "|" + s_hps[2] + "|" +  s_hps[3];
  s_estadao.prop5 = s_hps[0] + "|" + s_hps[1] + "|" + s_hps[2] + "|" +  s_hps[3] + "|" +  s_hps[4];
  s_estadao.eVar1 = s_estadao.pageName;
  s_estadao.prop6 = s_estadao.eVar6 = ppn;
  s_estadao.prop7 = s_estadao.eVar7 = visnum;
  s_estadao.prop8 = getTimeParting;
  s_estadao.prop9 = getDateParting;
  s_estadao.prop10 = s_estadao.eVar10 = newRepeat;
  s_estadao.eVar11 = ppv1;
  s_estadao.eVar12 = ppv2;
  s_estadao.eVar13 = ppv3;
  s_estadao.eVar14 = ppv4;
  s_estadao.eVar15 = ppv5;
  s_estadao.prop75 = "Estadao - 24/09/2014 - Ricardo";  // Code Version
...
s_estadao.getValOnce=new Function("v","c","e",""
+"var s=this,k=s.c_r(c),a=new Date;e=e?e:0;if(v){a.setTime(a.getTime("
+")+e*86400000);s.c_w(c,v,e?a:0);}return v==k?'':v");

s_estadao.split=new Function("l","d",""
 ...
 */
s_estadao.getPreviousValue=new Function("v","c","el",""
...
/*
 * Plugin: getTimeParting 3.4 - Modified by Adam Klintworth 2/11/13
 Changes include forcing timestamp to nearest half-hour, returning timestamp from server, and returning date timestamp as well.
 */
s_estadao.getTimeParting=new Function("h","z",""
...
+"imezoneOffset()*60000);d=new Date(d+(3600000*z));var dayOfMonth=d.g"
+"etDate();var monthOfYear=d.getMonth()+1;var thisYear=d.getFullYear("
+");H=d.getHours();M=d.getMinutes();if(M<30){M='00';}else if(M>30){M="
+"'30';}D=d.getDay();U=' AM';if(H>=12){U=' PM';H=H-12;}if(H==0){H=12;"
+"}D=da[D];tm=H+':'+M+U;if(h=='j'){return monthOfYear+'/'+dayOfMonth+"
+"'/'+thisYear;}else{return(D+'|'+tm);}}");

/*
 * Plugin: getNewRepeat 1.2 - Returns whether user is new or repeat
 */
s_estadao.getNewRepeat=new Function("d","cn",""
...
s_estadao.crossVisitParticipation=new Function("v","cn","ex","ct","dl","ev","dv",""
 ...
 */
s_estadao.apl=new Function("l","v","d","u",""
 ...
 */
s_estadao.getPercentPageViewed=new Function("n",""
...
/*                                                                 
   ...
   */
   s_estadao.getVisitNum=new Function("tp","c","c2",""
  ...
  s_estadao.dimo=new Function("m","y",""
  +"var d=new Date(y,m+1,0);return d.getDate();");
  s_estadao.endof=new Function("x",""
